...
CIT_Carrara_ARF: float = 0.3115 + 0.092  # valor adicionado de 0,092 que é a correção de ácido de 90ºC antiga(verificar)
...
TempConversion_A = 0.0391
TempConversion_B = 0.154

# TempConversion_A = 0.0383
# TempConversion_B = 0.258

# TempConversion_A = 0.0422
# TempConversion_B = 0.1262 # old script, Boniface et al. 2017 (-1<T<300°C carbonates)

# TempConversion_A = 0.0428
# TempConversion_B = 0.1174 # Boniface et al. 2017 (dolomites)
...
class CI_VALUE(object):
    """subclass defining how important isotopic ratios are calculated, and d18O_mineral, and brand (2010)-style d13C
    and d18O calcs"""
    ...
    def __get__(self, instance, cls):
        if len(instance.acqs) >= 1:
            if self.name in ['d13C', 'd13C_stdev', 'd18O_gas', 'd18O_stdev', 'd45', 'd46', 'd47', 'd47_stdev',
                             'D47_raw', 'D47_stdev', 'D47_sterr', 'd48', 'd48_stdev', 'd49', 'D48_raw', 'D48_stdev',
                             'd13C_brand',
...
class ACQUISITION(object):
    """A class for all the attributes of a single clumped isotope acquisition"""
...
def bulk_comp_brand_2010(acq, objName):
    ...
    # convert wg d18O to vpdb
    d18Oref_vpdb = (acq.d18Oref - (VPDB_VSMOW - 1) * 1000) / VPDB_VSMOW
    ...
    for l in range(len(R_measured_analysis)):
        delta_measured[l, :] = (R_measured_analysis[l, :] / (
                (R_measured_ref[l, :] + R_measured_ref[l + 1, :]) / 2) - 1) * 1000
    ...
    # From Daeron et al., 2016, Appendix A
    ...
    d18O_vsmow_brand = d18O_vpdb_brand * VPDB_VSMOW + (VPDB_VSMOW - 1) * 1000

    #assemble dict
    bulk_comps = {'d13C_brand': d13C_vpdb_brand, 'd18O_brand': d18O_vsmow_brand, 'd18O_taylor': d18O_taylor,
                  'd13C_taylor': d13C_taylor}
...
def CIDS_parser(filePath):
    ...
    for line in fileReader:
        ...
        if 'pre' in line:
            ...
            preIndex = line.index(
                'pre')  #need to save this so that following lines know where to start pulling voltages
...
def CIDS_cleaner(analyses, checkForOutliers=False):
    ...
    if not lowAcqs:
        ...
        print('\n'.join(
            ['Sample ' + str(k.num) + ': ' + k.name + ' has ' + str(len(k.acqs)) + ' acquisitions' for k in lowAcqs]))
    ...
    for i in range(len(analyses)):
        for j in range(len(analyses[i].acqs)):
            ...
            (analyses[i].acqs[j].d13C, analyses[i].acqs[j].d18O_gas) = np.around(
                (analyses[i].acqs[j].d13C, analyses[i].acqs[j].d18O_gas), 3)
...
def Check_for_wrong_acqs(analyses, showPlots=True):
    ...
    for i in range(len(analyses)):
        ...
        if G_2sided > crit_vals_2sided[len(d46_temp)]:
            print(('Sample {0} fails Grubbs\' outlier test at acq {1} with {2}-alpha significance'.format(i, np.argmax(
                d46_res), alpha)))
            if showPlots:
                ...
                # Plot d47
                ax1 = ax0.twinx()
                ax1.plot(np.asarray(list(range(len(acqs_temp)))) + analyses[i].skipFirstAcq, [j.d45 for j in acqs_temp],
                         'rd')
...
def FlatList_exporter(analyses, fileName, displayProgress=False):
    ...
    wrt.writerow(
        ['User', 'date', 'Type', 'Sample ID', 'spec #\'s', 'acqs', '100% bellow P (mbar)', 'd13C (vpdb)', 'd13C_stdev',
         'd18O_gas (vsmow)', 'd18O_mineral (vpdb)',
         'd18O_stdev', 'd47', 'd47_stdev', 'D47 (v. Oz)', 'D47_stdev', 'D47_sterr', 'd48', 'd48_stdev', 'D48',
         'D48_stdev', 'hg_slope', 'hg_intercept', 'D47_CRF', 'D47_ARF',
         'D47_ARF std error', 'mineral', 'rxnTemp', 'D47_ARF_acid', 'T(C)', 'D47_ARF_stdCorr', 'D48_excess',
         'd13C_brand', 'd18O_brand'])
    counter = 0
    if displayProgress:
        for item in analyses:
            wrt.writerow([item.user, item.date, item.type, item.name, item.num, (len(item.acqs) - item.skipFirstAcq),
                          item.acqs[0].pressureVals[0], item.d13C, item.d13C_stdev, item.d18O_gas, item.d18O_min,
                          item.d18O_stdev, item.d47, item.d47_stdev, item.D47_raw, item.D47_stdev, item.D47_sterr,
                          item.d48, item.d48_stdev, item.D48_raw, item.D48_stdev, item.hg_slope, item.hg_intercept,
                          item.D47_CRF, np.around(item.D47_ARF, 5), np.around(item.D47_error_all, 5), item.mineral,
                          item.rxnTemp, item.D47_ARF_acid, item.T_D47_ARF, item.D47_ARF_stdCorr, item.D48_excess,
                          item.d13C_brand, item.d18O_brand])
            counter += 1
    ...
    for analysis in analyses:
        ...
        for acq in analysis.acqs:
            ...
            wrt.writerow(['', 'acq num', 'date', 'ref gas d13C', 'ref gas d18O', 'name', 'type', 'd13C (vpdb)',
                          'd18O_gas (vsmow)', 'd45', 'd46', 'd47', 'd48', 'D47_raw', 'D48_raw', '100% bellow P',
                          '16V sample P', '16V wg P'])
            wrt.writerow(
                ['__AcqInfo__', acq.acqNum, acq.date, acq.d13Cref, acq.d18Oref, analysis.name, analysis.type, acq.d13C,
                 acq.d18O_gas, acq.d45, acq.d46, acq.d47, acq.d48, acq.D47_raw, acq.D48_raw, acq.pressureVals[0],
                 acq.pressureVals[1], acq.pressureVals[2]])
        ...
        wrt.writerow([])
        wrt.writerow(
            ['', 'User', 'date', 'Type', 'Sample ID', 'spec #\'s', 'SkipFirstAcq', 'mineral', 'rxnTemp', 'd13C (vpdb)',
             'd13C_stdev', 'd18O_gas (vsmow)', 'd18O_mineral (vpdb)',
             'd18O_stdev', 'd47', 'd47_stdev', 'D47 (v. Oz)', 'D47_stdev', 'D47_sterr', 'd48', 'd48_stdev', 'D48',
             'D48_stdev'])
        wrt.writerow(['__SampleSummary__', analysis.user, analysis.date, analysis.type, analysis.name, analysis.num,
                      analysis.skipFirstAcq, analysis.mineral, analysis.rxnTemp, analysis.d13C, analysis.d13C_stdev,
                      analysis.d18O_gas, analysis.d18O_min, analysis.d18O_stdev, analysis.d47, analysis.d47_stdev,
                      analysis.D47_raw,
                      analysis.D47_stdev, analysis.D47_sterr, analysis.d48, analysis.d48_stdev, analysis.D48_raw,
                      analysis.D48_stdev])
        wrt.writerow(24 * ['---', ])
...
def CIDS_importer(filePath, displayProgress=False):
    ...
    for line in fileReader:
        ...
        if '__SampleSummary__' in line:
            ...
            [analyses[-1].user, analyses[-1].date, analyses[-1].type, analyses[-1].name] = line[
                                                                                           summaryIndex + 1:summaryIndex + 5]
...
def Isodat_File_Parser(fileName):
    ...
    while i < 200 and not (found13Cstart and found18Ostart):
        ...
        if not found13Cstart:
            ...
            testList1 = struct.unpack('ddd', buff[start13C:start13C + 8] + buff[start13C + 16:start13C + 16 + 8] + buff[
                                                                                                                   start13C + 32:start13C + 32 + 8])
            if testList1 == (1.0, 2.0, 3.0):
                found13Cstart = True

        if not found18Ostart:
            start18O = startEval + 1000 + i
            testList2 = struct.unpack('ddd', buff[start18O:start18O + 8] + buff[start18O + 16:start18O + 16 + 8] + buff[
                                                                                                                   start18O + 32:start18O + 32 + 8])
    ...
    for item in analyses:
        if item.type in ['hg', 'eg']:
            if properNames == 'y':
                ...
                while True:
                    choice = input('Is acq num: ' + str(
                        item.num) + ' with name: ' + item.name + ' a (h)eated gas, an (e)quilibrated gas?, or (s)kip? ')
                    if choice.lower() == 'h':
                        item.D47nominal = ''
                        item.TCO2 = 1000
                        item.type = 'hg'
    ...
    for item in analyses:
        if item.type == 'sample':
            wrt.writerow([item.type, item.name, item.d45, item.d46, item.d47, item.d48, item.d49,
                          item.d47_stdev / np.sqrt(len(item.acqs) - item.skipFirstAcq), 0.0, item.acqs[0].d13Cref,
                          (item.acqs[0].d18Oref - (VPDB_VSMOW - 1) * 1000) / VPDB_VSMOW,
                          item.D47_raw])
        else:
            wrt.writerow([item.type, item.name, item.d45, item.d46, item.d47, item.d48, item.d49,
                          item.d47_stdev / np.sqrt(len(item.acqs) - item.skipFirstAcq), 0.0, item.acqs[0].d13Cref,
...
def Pressure_Baseline_Processer(fileFolder):
    ...
    for i in range(len(A)):
        max44 = A[i][:, 2].max()
        halfHeight = np.nonzero(
            A[i][:, 2] > (max44 / 2))  #array of indices of all values with mass44 intensity is gt half of max
...
def D47_calculation_valued(acq, objName):
    ...
    if acq.useBrand2010:
        ...
        # Calculate working gas ratios using VPDB
        R18_ref = (d18Oref_vpdb / 1000 + 1) * R18_VPDB
        R17_ref = np.power((R18_ref / R18_VPDB), LAMBDA_17) * R17_VPDB

        d18O_gas_vpdb = (acq.d18O_gas - (VPDB_VSMOW - 1) * 1000) / VPDB_VSMOW
        # Calculate sample gas ratios using VPDB
        R18_sa = (d18O_gas_vpdb / 1000 + 1) * R18_VPDB
        R17_sa = np.power((R18_sa / R18_VPDB), LAMBDA_17) * R17_VPDB

    else:
        # Calculate working gas ratios using VSMOW
        R18_ref = (acq.d18Oref / 1000 + 1) * R18_VSMOW
        R17_ref = np.power((R18_ref / R18_VSMOW), LAMBDA_17) * R17_VSMOW

        # Calculate sample gas ratios using VSMOW
    ...
    calculatedCIValues = {'d45': d45, 'd46': d46, 'd47': d47, 'd48': d48, 'd49': d49, 'D47_raw': D47_raw,
                          'D48_raw': D48_raw}
...
def CI_averages_valued_individual(analysis, objName):
    ...
    props2 = ['d13C', 'd13C_stdev', 'd18O_gas', 'd18O_min', 'd18O_stdev',
              'd47', 'd47_stdev', 'D47_raw', 'D47_stdev', 'D47_sterr', 'd48', 'D48_raw', 'D48_stdev', 'd13C_brand',
              'd18O_brand']
    ...
    for i in range(len(analyses1)):
        for j in range(len(analyses1[i].acqs)):
            if not analyses1[i].acqs[j].D47_raw == analyses2[i].acqs[j].D47_raw:
                ...
                print(('analyses1 D47 = {0:.3f}, analyses2 D47 = {1:.3f}'.format(analyses1[i].acqs[j].D47_raw,
                                                                                 analyses2[i].acqs[j].D47_raw)))
                equalSoFar = False
        if not analyses1[i].D47_stdev == analyses2[i].D47_stdev:
            ...
            print(('1. D47_stdev = {0:.3f}, 2. D47_stdev = {0:.3f}'.format(analyses1[i].D47_stdev,
                                                                           analyses2[i].D48_stdev)))
    ...
    if choice == 's':
        ...
        for i in range(len(analyses)):
            if analyses[i].type in ['std', 'eg', 'hg', 'sample']:
                ...
                if ('cit carrara' in name) or ('tv03' in name) or ('nbs-19' in name) or ('nbs19' in name) or (
                        'gc-az' in name) or ('gc_az' in name) or ('tv04' in name) or ('eth-1' in name) or (
                        'eth-2' in name) or ('eth-3' in name) or ('eth-4' in name):
                    analyses[i].type = 'std'
                    continue
                elif name.startswith("25c eboc") or name.startswith("25c boc"):
                    analyses[i].type = 'eg'
                    analyses[i].rxnTemp = 25
                    analyses[i].mineral = 'gas'
                    continue
                elif name.startswith("eboc") or name.startswith("boc"):
                    analyses[i].type = 'hg'
                    analyses[i].rxnTemp = 25
                    analyses[i].mineral = 'gas'
                    continue
...
def CI_CRF_data_corrector(analyses, showFigures=True):
    ...
    if showFigures:
        ...
        plt.errorbar([CIT_Carrara_CRF for i in range(len(stds_Carrara))],
                     [j.D47_CRF - CIT_Carrara_CRF for j in stds_Carrara], yerr=[k.D47_sterr for k in stds_Carrara],
                     fmt='o')
        plt.errorbar([TV03_CRF for i in range(len(stds_TV03))], [j.D47_CRF - TV03_CRF for j in stds_TV03],
                     yerr=[k.D47_sterr for k in stds_TV03], fmt='o')
        plt.errorbar([GC_AZ_CRF for i in range(len(stds_GC_AZ))], [j.D47_CRF - GC_AZ_CRF for j in stds_GC_AZ],
                     yerr=[k.D47_sterr for k in stds_GC_AZ], fmt='o')
...
def CI_48_excess_checker(analyses, showFigures=False):
    ...
    hg_slope_48, hg_intercept_48, r_48, sm_48, sb_48, xc_48, yc_, ct_ = lsqcubic(np.asarray([i.d48 for i in hgs]),
                                                                                 np.asarray([i.D48_raw for i in hgs]),
                                                                                 np.asarray([i.d48_stdev for i in hgs]),
                                                                                 np.asarray([i.D48_stdev for i in hgs]))
    for i in analyses:
...
def CI_D47_to_temp_ARF(D47_ARF_acid):
    """Function to apply a temperature calibration"""
    # NOTE: Using D47_ARF that is not acid corrected, because this is how Magali Boniface does it
...
def CI_water_calibration(d18O_min, T_C):
    ...
    T_K = T_C + 273.15
    # The Kim and Oneil 1997 equation (calcite 10 < T < 40°C):
...
def CI_carb_water_calibration(d18O_min, T_C, mineral):
    ...
    T_K = T_C + 273.15
    if mineral == 'dolomite':
        # Horita 2014 calibration (dolomita 80 < T < 350°C)
        alpha_carb_H2O = np.exp((3.140 * (1e6 / T_K ** 2) - 3.14) / 1000)
    else:
        #O'neil 1969, Friedman and o'neil 1977 calibration (via horita)(calcite 0 < T < 500°C)
    ...
    if useCarbStandards:
        for i in analyses:
            daeronData.append({'label': i.name, 'd45': i.d45, 'd46': i.d46, 'd47': i.d47, 'd48': i.d48, 'd49': i.d49,
                               'sd47': i.d47_stdev / np.sqrt(len(i.acqs) - i.skipFirstAcq), 'D17O': 0.0,
                               'd13Cwg_pdb': i.acqs[0].d13Cref,
                               'd18Owg_pdbco2': (i.acqs[0].d18Oref - (VPDB_VSMOW - 1) * 1000) / VPDB_VSMOW,
...
def Daeron_data_processer(analyses, showFigures=False):
    ...
    stdCorrChoice = input(
        'Use CIT (c)arrara, (t)V04, or (b)oth, or (e)th 1 to 3 for carbonate std correction? ').lower()
    if stdCorrChoice == 'c':
        ...
        CarbonateCorrection = (CarraraCorrection * len(stds_Carrara) + TV04_Correction * len(stds_TV04)) / (
                len(stds_Carrara) + len(stds_TV04))
        print('Both CIT Carrara and TV03 correction (weighted average) is: ' + str(CarbonateCorrection))
    elif stdCorrChoice == 'e':
        CarbonateCorrection = (ETH_1_Correction * len(stds_ETH_1) + ETH_2_Correction * len(
            stds_ETH_2) + ETH_3_Correction * len(stds_ETH_3)) / (len(stds_ETH_1) + len(stds_ETH_2) + len(stds_ETH_3))
...
def CI_background_correction(instance, objName):
    ...
    try:
        ...
        voltSamTemp = instance.voltSam_raw - np.transpose(
            np.tile(instance.voltSam_raw[:, 0], (len(slopeArray), 1))) * slopeArray
        voltRefTemp = instance.voltRef_raw - np.transpose(
            np.tile(instance.voltRef_raw[:, 0], (len(slopeArray), 1))) * slopeArray
    except(IndexError):
        return 0
